...
pub fn test_tensor_sub() {
    ...
    let expected = mv1
        .iter()
        .zip(mv2.iter())
        .map(|(a, b)| linear_algebra::Matrix::matrix_sub(a, b))
        .collect::<Vec<_>>();

    let t3 = linear_algebra::Tensor::tensor_sub(&t1, &t2);
